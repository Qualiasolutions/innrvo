...
export const AppLayout: React.FC<AppLayoutProps> = ({
  ...
  return (
    <div className={`min-h-screen bg-[#020617] ${className}`}>
      ...
      {/* Back Button - hidden when sidebar is open, responsive positioning */}
      {showBackButton && !isNavOpen && (
        <button
          onClick={() => navigate(backTo)}
          className="fixed top-6 left-[4.5rem] sm:left-20 md:top-8 md:left-24 text-slate-600 hover:text-white transition-all flex items-center gap-2 sm:gap-3 group btn-press focus-ring rounded-full z-[100]"
        >
          <div className="w-10 h-10 sm:w-12 sm:h-12 min-w-[40px] min-h-[40px] sm:min-w-[44px] sm:min-h-[44px] rounded-full border border-white/[0.06] bg-white/[0.02] flex items-center justify-center group-hover:bg-white/[0.08] group-hover:scale-105 group-hover:border-sky-500/20 transition-all">
            <ICONS.ArrowBack className="w-4 h-4 sm:w-5 sm:h-5" />
          </div>
