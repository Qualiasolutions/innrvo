...
import { saveMeditationToHistory, type MeditationSaveData } from './utils';

/**
 * MeditationPlayer - Full-screen immersive meditation player
 *
 * Beautiful animated player with breathing circle, floating particles,
 * and integrated audio controls. Connects with Supabase for saving meditations.
 */
...
const MeditationPlayer: React.FC<MeditationPlayerProps> = memo(({
  isPlaying,
  currentTime,
  duration,
  onPlayPause,
  ...
  const handleSave = useCallback(async () => {
    ...
    setIsSaving(true);
    try {
      const saveData: MeditationSaveData = {
        script,
        voiceId,
        voiceName,
        meditationType,
      ...
      const success = await saveMeditationToHistory(saveData);
      if (success) {
        setIsSaved(true);
        onSaveSuccess?.();
      }
  ...
  }, [userId, isSaving, isSaved, script, voiceId, voiceName, meditationType, duration, onSaveSuccess]);
...
const FloatingParticles = memo(() => {
  // Generate particles once with useMemo to prevent recreation on re-renders
